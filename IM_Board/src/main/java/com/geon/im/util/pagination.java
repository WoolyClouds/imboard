...
public class pagination {
    ...
    private void makePaging() {
        ...
        if(1 < pageNo) this.setPrevPageNo(pageNo-1);	// 이전 페이지
        else this.setPrevPageNo(1);	// 현재 페이지가 1이면 1로 고정
        ...
        if(pageNo < finalPageNo) this.setNextPageNo(pageNo+1);	// 다음 페이지
        else this.setNextPageNo(finalPageNo);	// 현재 페이지가 마지막 페이지면 마지막 페이지로 고정
